...
class DbHandler
{
    ...
    {
        $sql = "select *
from (SELECT t1.id, t1.forwarded, t4.subject, t4.body, t1.created_date as received_date, t2.username as from_username, t2.email_address 
                as from_email, t1.is_read FROM 
                (SELECT id, message_id, from_user_id, created_date, forwarded, is_read FROM inbox  WHERE user_id = '$user_id' 
                AND is_trashed = 0
                ORDER BY created_date ASC) as t1 INNER JOIN 
                ...
                WHERE t2.user_id = t1.from_user_id and t4.message_id = t1.message_id) as p1
                
                UNION ALL
                
                SELECT * FROM(SELECT t2.inbox_id as id, t3.forwarded, t5.subject, t5.body, t2.created_date as received_date, t4.username as from_username, t4.email_address 
                as from_email, t3.is_read FROM 
                (SELECT id FROM sent WHERE user_id = '$user_id' AND is_trashed = 0 ORDER BY created_date ASC) as t1 INNER JOIN
                (SELECT inbox_id, sent_id, created_date FROM `replies` WHERE created_date IN(SELECT MAX(created_date) 
                FROM replies GROUP BY inbox_id) GROUP BY sent_id) as t2 INNER JOIN
                (SELECT id, message_id, user_id, created_date, forwarded, is_read FROM inbox WHERE is_trashed = 0) as t3 INNER JOIN
                (SELECT username, user_id, email_address FROM user_details) as t4 INNER JOIN 
                (SELECT message_id, body, subject FROM messages) as t5
                WHERE t1.id = t2.sent_id and t3.id = t2.inbox_id and t4.user_id = t3.user_id 
                and t5.message_id = t3.message_id GROUP BY t2.inbox_id) as p2 ORDER BY received_date DESC";
    ...
    public function getSentData($user_id)
    {
        $sql = "SELECT t1.id, t4.subject, t4.body, t1.created_date as received_date, t2.username as to_username, t2.email_address as 
                ...
                (SELECT id, message_id, to_user_id, created_date, forwarded FROM sent WHERE user_id = '$user_id' AND is_trashed = 0) 
                as t1 INNER JOIN
                (SELECT username, user_id, email_address FROM user_details) as t2 INNER JOIN
        ...
        while ($item = $inbox_data->fetch_assoc()) {
            $id = $item['id'];
            $sql = "SELECT body, created_date FROM `replies` WHERE sent_id = '$id' GROUP BY sent_id";
            $reply = $this->conn->query($sql);
    ...
    {
        $sql = "SELECT t1.id, t3.subject, t3.body, t1.created_date as received_date, t2.username as to_username, t2.email_address as to_email FROM
                (SELECT id, message_id, to_user_id, created_date FROM drafts WHERE user_id = '$user_id') as t1 INNER JOIN
                (SELECT username, user_id, email_address FROM user_details) as t2 INNER JOIN
                (SELECT message_id, subject, body FROM messages) as t3 WHERE t1.message_id = t3.message_id and t1.to_user_id = t2.user_id  
ORDER BY `received_date` DESC";
    ...
    {
        $sql = "SELECT t3.username, t1.forwarded, t2.subject, t2.body, t1.created_date FROM 
                (SELECT message_id, from_user_id as other_user_id, forwarded, created_date FROM inbox WHERE is_trashed = 1 AND user_id = '5') 
                as t1 INNER JOIN
                (SELECT message_id, subject, body FROM messages) as t2 INNER JOIN
                (SELECT user_id,username FROM user_details) as t3
                WHERE t1.message_id = t2.message_id AND t3.user_id = t1.other_user_id
                UNION ALL 
                SELECT t3.username, t1.forwarded, t2.subject, t2.body, t1.created_date FROM 
                (SELECT message_id, to_user_id as other_user_id, forwarded, created_date FROM sent WHERE is_trashed = 1 AND user_id = '5') 
                as t1,
                (SELECT message_id, subject, body FROM messages) as t2,
                (SELECT user_id,username FROM user_details) as t3
                WHERE t1.message_id = t2.message_id AND t3.user_id = t1.other_user_id";
    ...
    public function compose($user_id, $subject, $body, $emails, $forwarded, $draft, $attachment, $draft_id)
    {
        if($draft_id > 0){
            $sql = "DELETE FROM drafts WHERE id = $draft_id";
            $this->conn->query($sql);
        }
        $email = $emails[0];
        ...
        if ($result != NULL && $result['user_id'] != 0) {
            ...
            if ($draft == 0) {
                $sql = "INSERT INTO inbox (message_id, user_id, from_user_id, forwarded, is_read) VALUES  
                    ('$message_id','$to_user_id', '$user_id', '$forwarded', '0')";
                $this->conn->query($sql);
    ...
    public function reply($reply, $user_id, $inbox_ids, $attachment)
    {
        ...
        if ($user_id != NULL && $user_id > 0) {
            ...
            for ($j = 0; $j < $size; $j++) {
                $sql = "UPDATE inbox SET is_read = 0 WHERE id = '$inbox_ids[$j]'";
                $this->conn->query($sql);
    ...
    public function forward($body_user, $user_id, $inbox_ids, $emails)
    {
        ...
        $body = '---------- Forwarded message ----------' . '<br>' . 'From:' . $result['username'] . ', ' .
            $result['email_address'] . '<br>' . 'Subject:' . $subject . '<br>' . $body_initial . '<br>' .
            '<br>' . $body_user;
        ...
        if ($result1 != NULL && $result1['user_id'] != 0) {
            ...
            $sql = "INSERT INTO inbox (message_id, user_id, from_user_id, forwarded, is_read) VALUES  
                    ('$message_id','$to_user_id', '$user_id', '1', '0')";
            $this->conn->query($sql);
            $inbox_id = $this->conn->insert_id;
        ...
        if (sizeof($emails) > 1) {
            ...
            foreach (array_slice($emails, 1) as $email) {
                ...
                if ($result != NULL && $result['user_id'] != 0) {
                    ...
                    $sql = "INSERT INTO inbox (message_id, user_id, from_user_id, forwarded, is_read) VALUES  
                    ('$message_id','$to_user_id', '$user_id', '1', '1')";
                    $this->conn->query($sql);
                    $inbox_id = $this->conn->insert_id;
    ...
    public function message($id, $from, $user_id, $is_read)
    {
        if ($from == 1) {
            if($is_read != 0) {
            ...
            $sql = "SELECT t1.id as inbox_id,t4.subject, t4.body, t1.created_date as received_date, t2.username as from_username, 
                    t2.email_address as from_email FROM 
                (SELECT id, message_id, from_user_id, created_date FROM inbox WHERE id = '$id' AND is_trashed = 0) as t1 INNER JOIN 
                (SELECT username, user_id, email_address FROM user_details) as t2 INNER JOIN 
            ...
            $sql = "SELECT t1.id as sent_id, t5.id as inbox_id, t4.subject, t4.body, t1.created_date as received_date, t2.username as to_username, 
                    t2.email_address as from_email FROM 
                (SELECT id, message_id, to_user_id, created_date FROM sent WHERE id = '$id' AND is_trashed=0) as t1 INNER JOIN 
                (SELECT username, user_id, email_address FROM user_details) as t2 INNER JOIN 
                (SELECT message_id, body, subject FROM messages) as t4 INNER JOIN
                (SELECT id, user_id, message_id FROM inbox) as t5
                WHERE t2.user_id = t1.to_user_id and t4.message_id = t1.message_id 
                and t5.user_id = t1.to_user_id and t1.message_id = t5.message_id";
            ...
            $sql = "SELECT t4.subject, t4.body, t1.created_date as received_date, t2.username as to_username, 
                    t2.email_address as to_email FROM 
                (SELECT id, message_id, to_user_id, created_date FROM drafts WHERE id = '$id') as t1 INNER JOIN 
                (SELECT username, user_id, email_address FROM user_details) as t2 INNER JOIN 
                (SELECT message_id, body, subject FROM messages) as t4 
                WHERE t2.user_id = t1.to_user_id and t4.message_id = t1.message_id";
    ...
    public function move_to_trash($inbox_ids, $source)
    {
        if ($source == 1) {
            for ($i = 0; $i < sizeof($inbox_ids); $i++) {
                $sql = "UPDATE inbox SET is_trashed = 1 WHERE id = '$inbox_ids[$i]'";
                $this->conn->query($sql);
            }
        } elseif ($source == 2) {
            for ($i = 0; $i < sizeof($inbox_ids); $i++) {
                $sql = "UPDATE sent SET is_trashed = 1 WHERE id = '$inbox_ids[$i]'";
                $this->conn->query($sql);
            }
        }
    }
